...
1. Assicurati di avere Node.js e npm installati sul tuo sistema. Puoi verificarlo eseguendo:
   ...
   npm -v
   ```
   Se non sono installati, scarica e installa Node.js dal sito ufficiale: [https://nodejs.org/](https://nodejs.org/).

2. Installa Gemini CLI globalmente utilizzando npm:
   ```bash
   npm install -g gemini-cli
   ```

3. Verifica che l'installazione sia avvenuta correttamente eseguendo:
   ```bash
   gemini --version
   ```

### Windows
1. Scarica e installa Node.js dal sito ufficiale: [https://nodejs.org/](https://nodejs.org/).

2. Apri un terminale (ad esempio, PowerShell o il Prompt dei comandi) e verifica che Node.js e npm siano installati:
   ```bash
   node -v
   npm -v
   ```

3. Installa Gemini CLI globalmente utilizzando npm:
   ```bash
   npm install -g gemini-cli
   ```

4. Verifica che l'installazione sia avvenuta correttamente eseguendo:
   ```bash
   gemini --version
   ```
...
1. Copia il file di esempio `settings.json` nella directory `.gemini` nella tua home directory:
   - Linux/macOS: `~/.gemini/settings.json`
   - Windows: `%USERPROFILE%\.gemini\settings.json`

2. Modifica il file `settings.json` e sostituisci `{OPENAPI_TOKEN}` con un token generato tramite [openapi.com](https://openapi.com):
   ```json
   {
     "mcpServers": {
       "OpenapiMcpServer": {
...
3. Salva il file e verifica che Gemini CLI sia configurato correttamente eseguendo:
   ```bash
   gemini --help
   ```
...
Se desideri fare una prova veloce, puoi modificare direttamente il file di esempio `settings.json` presente nella directory `.gemini` di questa repository. Non è necessario copiarlo nella tua home directory. Assicurati di sostituire `{OPENAPI_TOKEN}` con un token valido generato tramite [openapi.com](https://openapi.com):
...
Dopo aver modificato il file, puoi eseguire il comando `gemini` direttamente da questa directory per verificare che l'MCP di openapi.com sia abilitato.

## Esempio di utilizzo

### Lista comandi disponibili
Esegui il seguente comando per avviare Gemini CLI e visualizzare l'elenco dei comandi disponibili:

```bash
gemini --help
```

### Avvio in modalità cli e autenticazione
Per avviare il client in modalità cli è sufficiente il comando:
```bash
gemini
```
Verrà avviato il client, la prima volta verra richiesto di autenticarsi:
```bash
...
```
### Verifica della connessione ad mcp.openapi.com
Notare la notifica "1 MCP server (ctrl+t to view)" nella schermata di benvenuto del client.
Premendo la combinazione suggerita verrà eseguito il comando:
```bash
/mcp desc 
```
che mostrerà il contesto fornito dal server MCP e premendola una seconda volta verrà mostrato l'elenco dei tools disponibili nel server MCP ovvero l'output del comdando:
```bash
/mcp nodesc
...
Se il token inserito è valido possiamo iniziare a "parlare" in maniera naturale con i servizi integrati di openapi.com attraverso il modello di linguaggio selezionato. In questa dimostrazione è selezionato gemini-2.5-pro, consultare la documantazione del client per un approfondimento sui modelli disponibili e i loro costi e disponibilità.
Facciamo semplice domanda al modello:
```bash
╭──────────────────────────────────────────────────────────╮
│  > in tasca ho 12 euro, quanto valgono oggi in dollari?  │
╰──────────────────────────────────────────────────────────╯
```
Dopo poco il modello selezionerà il correto servizio per rispondere alla domanda in maniera esatta evitando allucinazioni.
```bash
 ╭─────────────────────────────────────────────────────────────────────────────────────────╮
